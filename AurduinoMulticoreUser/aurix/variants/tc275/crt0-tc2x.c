...
static void init_csa(unsigned int csa_base, unsigned int csa_size)
{
	...
	while (csa_size--)
	{
		...
		/* Make sure CSA's are all zeroed */
		pcsa->reg[1] = 0x00000000ul;
		pcsa->reg[2] = 0x00000000ul;
		pcsa->reg[3] = 0x00000000ul;
		pcsa->reg[4] = 0x00000000ul;
		pcsa->reg[5] = 0x00000000ul;
		pcsa->reg[6] = 0x00000000ul;
		pcsa->reg[7] = 0x00000000ul;
		pcsa->reg[8] = 0x00000000ul;
		pcsa->reg[9] = 0x00000000ul;
		pcsa->reg[10] = 0x00000000ul;
		pcsa->reg[11] = 0x00000000ul;
		pcsa->reg[12] = 0x00000000ul;
		pcsa->reg[13] = 0x00000000ul;
		pcsa->reg[14] = 0x00000000ul;
		pcsa->reg[15] = 0x00000000ul;
...
{
	TRAP_ENTRY(trap_function); // Trap_memoryManagementError
	TRAP_ENTRY(trap_function); // Trap_internalProtectionError
	TRAP_ENTRY(trap_function); // Trap_instructionError
	TRAP_ENTRY(trap_function); // Trap_contextManagementError
	TRAP_ENTRY(trap_function); // Trap_busError
	TRAP_ENTRY(trap_function); // Trap_assertion
	TRAP_ENTRY(trap_function); // Trap_systemCall_Cpu0
	TRAP_ENTRY(trap_function); // Trap_nonMaskableInterrupt
	while(1);
}
#pragma section
